...
class laceworksdk_host_vuln_dataset_handler(dataset_handler):
    def load(self):
        ...
        last_report = df.loc[
            (df['assessment_date'] >= last_report_start_time)
            & (df['assessment_date'] <= last_report_end_time)
            ]
        ...
        new = last_report.loc[
            (df['first_seen_time'] <= self.dataset.get('end_time'))
            & (df['first_seen_time'] >= self.dataset.get('start_time'))
            ]
        # all with fixed_time in the reporting period as of latest assessment
        fixed = last_report.loc[
            (df['status'] == 'Fixed')
            & (df['fixed_time'] <= self.dataset.get('end_time'))
            & (df['fixed_time'] >= self.dataset.get('start_time'))
            ]
        # all fixed both in an out of the reporting period
        all_fixed = last_report.loc[(df['status'] == 'Fixed')]

        severity_summary = df.loc[
            (df['status'] == 'Active')].set_index('assessment_date').groupby([
                pd.Grouper(freq='d'),
                'severity'],
                as_index=False).size().rename(columns={"size": "count"})
        status_summary = df.set_index('assessment_date').groupby([
            pd.Grouper(freq='d'),
            'severity', 'status'],
            as_index=False).size().rename(columns={"size": "count"})
